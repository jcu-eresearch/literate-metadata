...
<!-- Test that every CharacterString element has content or it's parent has a
    ...
    <sch:pattern fpi="CharacterString must have content"><sch:title>CharacterString must have content or it's parent
        must have a valid nilReason attribute. Doesn't test if CharacterString
        exists for all relevant parents.</sch:title>
        <sch:rule context="//*[../gco:CharacterString]">
            <sch:report test="(normalize-space(../gco:CharacterString) eq '') and
                not(../@gco:nilReason = $nilReasonList)">CharacterString must
                have content or parent's
                nilReason attrbute must be legitimate.</sch:report>
        </sch:rule>
    </sch:pattern>
